...
public class Playermovement2 : MonoBehaviour
{
    ...
    void ClicktoMove() {
        if (Input.GetMouseButtonDown(0) && turnScript.isWaitForPlayer2 == true)
        {
    ...
    IEnumerator WaitRotate() {
        ...
        RotateTrap[] P2Enemy = GameObject.Find("P2Trap").gameObject.GetComponentsInChildren<RotateTrap>();
        if (P2Enemy != null)
        {
            for (int i = 0; i < P2Enemy.Length; i++)
            {
                P2Enemy[i].gameObject.GetComponent<RotateTrap>().RotatethisTrap();
            }
        }
    }
