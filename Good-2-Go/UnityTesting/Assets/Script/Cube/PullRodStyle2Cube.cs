...
public class PullRodStyle2Cube : MonoBehaviour
{
    ...
    {
        if (gameObject.transform.position == Pos1.transform.position && couldMove ==true )
        {
            gofirstPos1 = false;
        ...
        if (gameObject.transform.position == Pos2.transform.position && couldMove == true)
        {
            gofirstPos1 = true;
            source.clip = triggersounds[Random.Range(0, triggersounds.Length)];
            source.PlayOneShot(source.clip);
